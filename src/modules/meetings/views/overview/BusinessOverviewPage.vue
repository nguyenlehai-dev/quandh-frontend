...
const upcomingMeetings = computed(() => {
  ...
  return meetingsData.value.data
    .filter(item => item.status === 'pending' || item.status === 'in_progress' || item.status === 'active')
    .slice(0, 5)
})

const statusLabels = ['Pending', 'In progress', 'Completed', 'Canceled']
...
<template>
  <div class="business-overview">
      ...
      <div class="d-flex align-center gap-3">
        ...
        <div>
          ...
          <div class="text-body-2 text-disabled">
            High-level summary of meeting activity in the system
          </div>
        </div>
      </div>

      <VBtn
        v-if="canViewMeetingList"
        color="primary"
    ...
    <VAlert
      v-if="!canViewMeetingList && !canViewMeetingStats"
      type="info"
      variant="tonal"
      class="mb-6"
    >
    ...
    <template v-else>
      ...
      <VRow v-if="canViewMeetingList">
        <VCol cols="12">
          <VCard
            title="Upcoming or active meetings"
            subtitle="Quick view of meetings that need attention"
          >
            <template #append>
              <VBtn
                variant="tonal"
