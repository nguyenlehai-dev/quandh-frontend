...
const submitForm = async () => {
  if (!formData.value.name) {
    showSnackbar('Vui lòng nhập tên lĩnh vực.', 'warning')
  ...
  try {
    if (isEditDialogVisible.value) {
      ...
      showSuccess('Cập nhật lĩnh vực thành công.')
    } else {
      await createDocumentField(formData.value)
      isAddDialogVisible.value = false
      showSuccess('Tạo lĩnh vực thành công.')
    }
    fetchItems()
  } catch (err) {
    showError(err, 'Không thể lưu lĩnh vực.')
...
const deleteItem = item => {
  openConfirmDialog({
    ...
    confirmColor: 'error',
    action: async () => {
      await deleteDocumentField(item.id)
      showSuccess('Xóa lĩnh vực thành công.')
      fetchItems()
    },
  })
}
...
const bulkDelete = () => {
  ...
  openConfirmDialog({
    title: 'Xoa hang loat linh vuc',
    message: `Ban co chac chan muon xoa ${selectedRows.value.length} linh vuc da chon khong?`,
    confirmText: 'Xoa',
    confirmColor: 'error',
    action: async () => {
      await bulkDeleteDocumentFields({ ids: selectedRows.value })
      selectedRows.value = []
      showSuccess('Xoa hang loat linh vuc thanh cong.')
      fetchItems()
    },
  })
}
...
const confirmBulkUpdateStatus = async () => {
  ...
  try {
    ...
    showSuccess('Cap nhat trang thai hang loat linh vuc thanh cong.')
    fetchItems()
  } catch (err) {
...
const toggleItemStatus = item => {
  ...
  openConfirmDialog({
    ...
    action: async () => {
      ...
      showSuccess('Đổi trạng thái lĩnh vực thành công.')
      fetchItems()
    },
  })
}
...
const importData = async () => {
  ...
  try {
    ...
    importFile.value = []
    showSuccess('Import linh vuc tai lieu thanh cong.')
    fetchItems()
  } catch (error) {
    showError(error, 'Khong the import linh vuc tai lieu.')
  } finally {
    isSubmitting.value = false
  }
}
...
<template>
  <section>
    ...
    <VDialog
      ...
      max-width="480"
    >
      <VCard title="Nhap linh vuc tai lieu">
        <VCardText>
          <VFileInput
            v-model="importFile"
