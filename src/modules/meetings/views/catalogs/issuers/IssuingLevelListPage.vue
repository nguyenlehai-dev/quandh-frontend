...
import { deleteIssuingLevel, createIssuingLevel, updateIssuingLevel, exportIssuingLevels, changeIssuingLevelStatus } from '@/modules/meetings/services/meetingService'
...
const headers = [
  { title: 'Tên Cấp ban hành', key: 'name' },
  { title: 'Mô tả', key: 'description' },
  { title: 'Trạng thái', key: 'status' },
  { title: 'Hành động', key: 'actions', sortable: false },
]

const { data: requestData, execute: fetchItems, isFetching: isLoading } = useApi(createUrl('/issuing-levels', {
  query: {
...
const submitForm = async () => {
  if (!formData.value.name) {
    showSnackbar('Vui lòng nhập tên cấp ban hành.', 'warning')
  ...
  try {
    if (isEditDialogVisible.value) {
      await updateIssuingLevel(selectedItemId.value, formData.value)
      isEditDialogVisible.value = false
      showSuccess('Cập nhật cấp ban hành thành công.')
    } else {
      await createIssuingLevel(formData.value)
      isAddDialogVisible.value = false
      showSuccess('Tạo cấp ban hành thành công.')
    }
    fetchItems()
  } catch (err) {
    showError(err, 'Không thể lưu cấp ban hành.')
...
const deleteItem = item => {
  openConfirmDialog({
    title: 'Xóa cấp ban hành',
    message: `Bạn có chắc chắn muốn xóa "${item.name}" không?`,
    confirmText: 'Xóa',
    confirmColor: 'error',
    action: async () => {
      await deleteIssuingLevel(item.id)
      showSuccess('Xóa cấp ban hành thành công.')
      fetchItems()
    },
  })
}
...
const toggleItemStatus = item => {
  ...
  openConfirmDialog({
    title: 'Đổi trạng thái cấp ban hành',
    message: `Bạn có chắc chắn muốn chuyển "${item.name}" sang trạng thái "${nextLabel}" không?`,
    confirmText: 'Đổi trạng thái',
    confirmColor: 'warning',
    action: async () => {
      await changeIssuingLevelStatus(item.id, nextStatus)
      showSuccess('Đổi trạng thái cấp ban hành thành công.')
      fetchItems()
    },
  })
}
...
const exportData = async () => {
  ...
  try {
    const res = await exportIssuingLevels({
    ...
    downloadBlob(res, 'cap-ban-hanh.xlsx')
  } catch (error) {
    showError(error, 'Không thể xuất dữ liệu cấp ban hành.')
...
<template>
  <section>
      ...
      <div class="pa-5">
        <VRow>
          ...
          >
            ...
            <AppTextField
              v-model="searchQuery"
              placeholder="Tìm kiếm cấp ban hành..."
              density="compact"
            />
          </VCol>
          <VCol
    ...
    <VDialog
      ...
      max-width="500"
    >
      <VCard title="Thêm Cấp ban hành">
        <VCardText>
          <VRow>
            <VCol cols="12">
              <AppTextField
                v-model="formData.name"
                label="Tên cấp ban hành"
                required
              />
            </VCol>
            <VCol cols="12">
    ...
    <VDialog
      ...
      max-width="500"
    >
      <VCard title="Cập nhật Cấp ban hành">
        <VCardText>
          <VRow>
            <VCol cols="12">
              <AppTextField
