...
export function useMeetingTypeListPage() {
  ...
  // Data fetching
  const { data: requestData, execute: fetchItems, isFetching: loading } = useApi(createUrl('/meeting-types', {
    query: {
  ...
  const bulkChangeStatus = async status => {
    if (!selectedRows.value.length) return
    try {
      await bulkUpdateMeetingTypes({ ids: selectedRows.value, status })
      selectedRows.value = []
