...
const canViewRoles = computed(() => ability.can('index', 'Role'))
...
const fetchReferences = async () => {
  try {
    const [rolesRes, orgsRes] = await Promise.allSettled([
      canViewRoles.value ? $api('/roles?limit=100') : Promise.resolve(null),
      canViewOrganizations.value ? $api('/organizations?limit=100') : Promise.resolve(null),
    ])
